#pragma once

//	When calculating many mathematical functions,
//	it is recommended to use the setprecision()
//	function from the <iomanip> library:
//  std::cout << std::fixed << std::setprecision(10);

#include "MathFunctions/Factorial.hpp"
#include "MathFunctions/Subfactorial.hpp"
#include "MathFunctions/Exp.hpp"
#include "MathFunctions/Trigonometry/Sin.hpp"
#include "MathFunctions/Trigonometry/Cos.hpp"
#include "MathFunctions/Trigonometry/Tan.hpp"
#include "MathFunctions/Trigonometry/Ctg.hpp"
